package com.agx.aicodemother.core;

import cn.hutool.json.JSONUtil;
import com.agx.aicodemother.ai.AiCodeGeneratorService;
import com.agx.aicodemother.ai.AiCodeGeneratorServiceFactory;
import com.agx.aicodemother.ai.model.HtmlCodeResult;
import com.agx.aicodemother.ai.model.MultiFileCodeResult;
import com.agx.aicodemother.ai.model.message.AiResponseMessage;
import com.agx.aicodemother.ai.model.message.ToolExecutedMessage;
import com.agx.aicodemother.ai.model.message.ToolRequestMessage;
import com.agx.aicodemother.core.parser.CodeParserExecutor;
import com.agx.aicodemother.core.saver.CodeFileSaverExecutor;
...
/**
 * 使用门面模式，让客户端只需要与这些简化的接口交互，不用了解内部复杂的实现细节
 */
@Service
@Slf4j
    ...
    public File generateAndSaveCode(String userMessage, CodeGenTypeEnum codeGenType, Long appId) {
        ...
        return switch (codeGenType) {
            case HTML -> {
                ...
                yield CodeFileSaverExecutor.executeSaver(codeResult, CodeGenTypeEnum.HTML, appId);
            }
            case MULTI_FILE -> {
                MultiFileCodeResult codeResult = aiCodeGeneratorService.generateMultiFileCode(userMessage);
                yield CodeFileSaverExecutor.executeSaver(codeResult, CodeGenTypeEnum.MULTI_FILE, appId);
            }
            default -> {
